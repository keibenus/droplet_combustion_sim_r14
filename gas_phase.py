...
def calculate_gas_advection_rhs(
    ...
    for i in range(Ng):
        ...
        if i == 0:
            print(f"   adv  u_g_faces[i]={u_g_faces[i]:.4e} AdvFluxRate_Yk[fuel,0]={AdvFluxRate_Yk[96, i]:.4e} net_adv_Yk_rate={net_adv_Yk_rate[96]:.4e} mass_flux_rate_0={mass_flux_rate_0:.4e} dY_dt_adv[fuel,i]={dY_dt_adv[96, i]:.4e}")
...
def build_diffusion_coefficients_gas_Y(
    ...
    # +++ デバッグ出力追加 +++
    fuel_idx_local = 96 # configから直接ではなくgas_props経由で取得 !!注意、ハードコーディング
